...
if ($order_id and $amount) {
    ...
    $this->getUser()->setFlash('message', $this->getContext()->getI18N()->__('Your Selected Agent Company is Refill Successful %2%%1% is approved',array('%1%'=>$amount,'%2%'=>sfConfig::get('app_currency_code'))));
...
public function executeAgentCompanyPayment(sfWebrequest $request) {
  ...
  if ($order_id and $amount) {
    ...
    $this->getUser()->setFlash('message', 'Your Selected Agent Company is Charged Successfully ' . sfConfig::get('app_currency_code') .-$amount . ' is approved');
