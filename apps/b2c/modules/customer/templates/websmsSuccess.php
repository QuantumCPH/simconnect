...
<?php if (!($balance <= 0.00 )){ ?>
          <form action=<?php echo url_for('customer/websms', true) ?>  method="post" id="websms" onsubmit="isHex(this.value)">
              <h3 style="width: 400px;"><?php echo __("Your account balance is") ?>: <?php echo sfConfig::get('app_currency_code')?><?php echo $balance ?></h3>
   <?php }else{ ?>
                
                <h3 style="width: 400px;"><?php echo __("Your Current Balance is:") ?> <?php echo sfConfig::get('app_currency_code')?><?php echo $balance ?></h3>
